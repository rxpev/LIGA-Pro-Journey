      ...
      <div className="grid grid-cols-3 gap-6 w-full max-w-4xl items-stretch">
        {[
          ...
          <div
            ...
            className={cx(
              'cursor-pointer transition-all duration-200 rounded-2xl shadow-lg backdrop-blur-md',
              'border-2 h-full flex flex-col items-center p-7',
              selectedRole === r.role
                ? 'border-primary bg-base-200/60'
                : 'border-white/10 hover:border-primary/40 bg-base-200/30',
            )}
          >
            <div
              className={cx(
                ...
                selectedRole === r.role
                  ? 'bg-gradient-to-br from-[#fb923c] via-[#f97316] to-[#ea580c] shadow-[0_0_20px_rgba(249,115,22,0.7)]'
            ...
            >
              <img
                src={r.role === 'RIFLER' ? riflerIcon : r.role === 'AWPER' ? awperIcon : iglIcon}
                alt={r.label + ' icon'}
