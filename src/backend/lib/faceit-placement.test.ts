...
const killEvent = (attackerId: number, victimId: number) => ({
  payload: JSON.stringify({ type: 'playerkilled' }), attackerId, victimId,
});
...
async function checkIntegrity(elo: number, matches: Array<{ faceitEloDelta: number | null; faceitRating: number | null }>) {
...
async function main() {
  ...
  assert.equal(calculatePlacement([match(1.5, 1500), match(1.5, 1500, false), match(1.5, 1500)], 1), 1550);
  ...
  const rebound = calculatePlacement([match(1.3, 1400), match(0.8, 2001, false), match(1.5, 1600)], 1);
  assert.ok(rebound >= 1550 && rebound <= 1800, `rebound Elo: ${rebound}`);

  assert.equal((await checkIntegrity(0, [])).valid, true);
  assert.equal((await checkIntegrity(0, [{ faceitEloDelta: null, faceitRating: null }])).valid, true);
  assert.equal((await checkIntegrity(1200, [])).valid, true);
  assert.equal((await checkIntegrity(1225, [{ faceitEloDelta: 25, faceitRating: null }])).valid, true);
  ...
  assert.equal((await checkIntegrity(1625, [...placed, { faceitEloDelta: 25, faceitRating: null }])).valid, true);
